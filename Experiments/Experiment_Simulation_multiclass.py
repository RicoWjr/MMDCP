...
mean1 = np.repeat(0,p)
mean2 = np.concatenate((np.repeat(1.3,d),np.repeat(1.3,p-d)))
mean3 = np.concatenate((np.repeat(-1.3,d),np.repeat(-1.3,p-d)))
mean4 = np.concatenate((np.repeat(2.5,d),np.repeat(2.5,p-d)))
mean0 = np.concatenate((np.repeat(0,d),np.repeat(0,p-d)))
mean = [mean1, mean2, mean3, mean4, mean0]

covv = 0
cov1 = np.diag(np.repeat(1,p))
cov2 = np.diag(np.concatenate((np.repeat(1,d),np.repeat(1, p-d))))
cov3 = np.diag(np.concatenate((np.repeat(1,d),np.repeat(1, p-d))))
cov4 = np.diag(np.concatenate((np.repeat(1,d),np.repeat(1, p-d))))
cov0 = np.diag(np.concatenate((np.repeat(2.5,d),np.repeat(2.5, p-d))))
for c in [cov1,cov2,cov3,cov4,cov0]:
    for i in range(len(c)):
        for j in range(len(c)):
            if i!=j:
                c[i][j] = covv
cov = [cov1, cov2, cov3, cov4, cov0]
...
res['SCwise FDR'] = np.average(np.array(scwise_fdr_loop), axis = 0)
...
res['SCwise FDR_std'] = np.std(np.array(scwise_fdr_loop), axis = 0)
